  ...
  PUBLIC SECTION.
    ...
    METHODS:
      ...
      set_logical_file_name
        IMPORTING
          iv_lname TYPE FILENAME-FILEINTERN
        RETURNING
...
CLASS ZKH_AS_FILE_UPLOAD IMPLEMENTATION.
  ...
  METHOD upload_file.
    ...
    lv_file_path = me->MV_PHYSICAL_FILE.
    IF me->check_file_exists( iv_file_path = lv_file_path ) = abap_true.
      ...
      ENDIF.
    ENDIF.
    IF NOT me->save_file( iv_file_path = iv_source iv_target_path = iv_target ).
      gv_subrc = 1.
    ELSE.
      gv_subrc = 0.
  ...
  METHOD set_logical_file_name.
    ...
    IF sy-subrc <> 0.
      ...
      rv_target_path = ''.
    ELSE.
      rv_target_path = MV_PHYSICAL_FILE.
    ENDIF.
  ENDMETHOD.
